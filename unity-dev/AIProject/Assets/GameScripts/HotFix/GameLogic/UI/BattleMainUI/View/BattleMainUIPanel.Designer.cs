// <auto-generated>
// This code was generated by FairyGUI GenCode plugin.
// Do not modify manually - changes will be overwritten.
// </auto-generated>

using FairyGUI;

namespace GameLogic
{
    /// <summary>
    /// BattleMainUIPanel 窗口设计类 - 自动生成，请勿手动修改。
    /// 业务逻辑请在 BattleMainUIPanel.cs 中实现。
    /// </summary>
    public partial class BattleMainUIPanel : FairyUIWindow
    {
        /// <summary>
        /// 包名称常量。
        /// </summary>
        public const string PackageNameConst = "BattleMainUI";

        /// <summary>
        /// 组件名称常量。
        /// </summary>
        public const string ComponentNameConst = "BattleMainUIPanel";

        /// <inheritdoc/>
        public override string PackageName => PackageNameConst;

        /// <inheritdoc/>
        public override string ComponentName => ComponentNameConst;

        #region UI Fields

        public CompMainUserInfo CompUserInfo;
        public Controller CtrlJoystick;
        public Controller CtrlMatch;
        public Controller CtrlMulti;
        public Controller CtrlPage;
        public Controller CtrlShowNpcTips;
        public Controller CtrlTaskTips;
        public Transition TransExitSwimming;
        public Transition TransHide;
        public Transition TransPlayNpc;
        public Transition TransShow;

        #endregion

        /// <inheritdoc/>
        protected override void BindMemberProperty()
        {
            base.BindMemberProperty();

            // 自动绑定 UI 组件字段
            CompUserInfo = (CompMainUserInfo)GetChild("compUserInfo");
            CtrlJoystick = GetController("ctrlJoystick");
            CtrlMatch = GetController("ctrlMatch");
            CtrlMulti = GetController("ctrlMulti");
            CtrlPage = GetController("ctrlPage");
            CtrlShowNpcTips = GetController("ctrlShowNpcTips");
            CtrlTaskTips = GetController("ctrlTaskTips");
            TransExitSwimming = GetTransition("transExitSwimming");
            TransHide = GetTransition("transHide");
            TransPlayNpc = GetTransition("transPlayNpc");
            TransShow = GetTransition("transShow");
        }
    }
}